...
const getPostsForHomepageQuery = `*[_type == "post"]{_id, title, slug, excerpt, mainImage, publishedAt} | order (publishedAt desc)`;
...
const getPostsForHome = async () => {
	console.log("getPostsForHome from db called");
	return await client.fetch(getPostsForHomepageQuery);
};
...
async function main() {
	...
	const homePosts = await getPostsForHome();
	...
	const jsonHome = JSON.stringify(homePosts);
	const bufferHome = Buffer.from(jsonHome);

	writePostsToFile("posts.json", bufferPosts);
	writePostsToFile("search.json", bufferSearch);
	writePostsToFile("home.json", bufferHome);
}
