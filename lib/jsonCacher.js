...
const getAllPosts = async () => {
	...
	const query =
		'*[_type == "post"]{...,body[]{..., _type=="reference"=>^->, asset->{..., "_key": _id} ,markDefs[]{..., _type == "internalLink" => {"slug": ^.reference->slug}}}}';
...
async function main() {
	...
	for (let i = 0; i < posts.length; i++) {
		// posts[i].body = toPlainText(posts[i].body);
		searchPosts[i].body = toPlainText(searchPosts[i].body);
	}
