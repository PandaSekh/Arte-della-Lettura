...
export default function Index({ authorBookJSON }: { authorBookJSON: string }) {
	const authorsBooksMap: Map<string, Array<BookTitleSlug>> = new Map(
    JSON.parse(authorBookJSON)
  );
	...
	authorsWithAlphabet.forEach((authors: Array<string>, char: string) => {
		...
		const authorsWithBooks: Map<string, Array<BookTitleSlug>> = new Map<
      string,
      Array<BookTitleSlug>
    >();
		...
		authorsWithBooks.forEach((books: Array<BookTitleSlug>, author: string) => {
          ...
          <li key={keygen()}>
            <Link href={book.reviewSlug}>
              <a className="text-customBlue hover:underline">{book.title}</a>
            </Link>
          </li>;
			});
			authorsMapped.push(
...
export const getStaticProps = async () => {
	...
	authors.forEach((author) =>
    authorAndBooks.set(author, getAuthorBookTitleSlug(stringToSlug(author)))
  );
